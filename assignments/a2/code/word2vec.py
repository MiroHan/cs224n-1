...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat = softmax(np.dot(outsideVectors, centerWordVec)).reshape(-1, 1)
    y = np.zeros(y_hat.shape)
    y[outsideWordIdx] = 1
    loss = -np.log(y_hat[outsideWordIdx])
    gradCenterVec = np.dot(outsideVectors.T, (y_hat - y))
    gradOutsideVecs = np.dot((y_hat - y), centerWordVec.reshape(1, -1))
...
def negSamplingLossAndGradient(
    ...
    sampleVectors = outsideVectors[indices]
    sampleVectors[0] *= -1
    loss = -np.log(sigmoid(-np.dot(sampleVectors, centerWordVec))).sum()
    gradCenterVec = np.dot(sampleVectors.T, sigmoid(
        np.dot(sampleVectors, centerWordVec)))
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] += - \
        sigmoid(
            np.dot(-outsideVectors[outsideWordIdx], centerWordVec)) * centerWordVec
    for idx in negSampleWordIndices:
        gradOutsideVecs[idx] += sigmoid(np.dot(outsideVectors[idx],
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWordIdx in outsideWordIndices:
        loss, currentGradCenterWord, currentGradOutsideVectors = word2vecLossAndGradient(centerWordVectors[centerWordIdx],
                                                                                         outsideWordIdx,
                                                                                         outsideVectors,
                                                                                         dataset)
        total_loss += loss 
        gradCenterVecs[centerWordIdx] += currentGradCenterWord.reshape(-1, )
