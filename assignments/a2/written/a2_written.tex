...
\newcommand{\hmwkAuthorName}{Thanh-Do Nguyen} % Your name
              ...
              \begin{answer}
                  ...
                  \begin{enumerate}[label=(\roman*)]
                      \item \begin{itemize}
                                ...
                                \item
                                      \begin{equation}
                                          \frac{\partial}{\partial \bm u_{w_{s}}}\bm J_{\text{neg-sample}}
                                          = -(1-\sigma(\bm u_{w_{s}}^\top \bm v_{c})\bm v_{c}
                                          = \sigma(\bm u_{w_{s}}^\top \bm v_{c})\bm v_{c}
                                      \end{equation}
                            \end{itemize}
                      \item I don't understand the question.
                      \item $ O(\text{Vocab}) >> O(K) $
                  \end{enumerate}
              \end{answer}
          \end{shaded}
    ...
    \item (2 points) Now we will repeat the previous exercise, but without the assumption that the $K$ sampled words are distinct.  Assume that $K$ negative samples (words) are drawn from the vocabulary. For simplicity of notation we shall refer to them as $w_1, w_2, \dots, w_K$ and their outside vectors as $\bm u_{w_1}, \dots, \bm u_{w_K}$. In this question, you may not assume that the words are distinct. In other words, $w_i=w_j$ may be true when $i\neq j$ is true.
          ...
          \begin{shaded}
              \begin{answer}
                  \begin{equation}
                      \frac{\partial}{\partial \bm u_{w_{s}}}\bm J_{\text{neg-sample}}
                      = \sigma(\bm u_{w_{s}}^\top \bm v_{c})\bm v_{c} \sum_{s' = 1}^K \bm 1(w_{s} = w_{s'})
                  \end{equation}
              \end{answer}
          \end{shaded}
