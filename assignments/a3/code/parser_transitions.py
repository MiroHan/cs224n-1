...
def minibatch_parse(sentences, model, batch_size):
    ...
    while (len(unfinished_parses) != 0):
        # 1. Take the first batch size parses in unfinished parses as a minibatch
        parses = unfinished_parses[:batch_size]

        # 2. Use the model to predict the next transition for each partial parse in the minibatch
        transitions = model.predict(parses)

        for i, parse in enumerate(parses):
            # 3. Perform a parse step on each partial parse in the minibatch with its predicted transition
            parse.parse_step(transition=transitions[i])

            # 4. Remove the completed (empty buffer and stack of size 1) parses from unfinished parses
