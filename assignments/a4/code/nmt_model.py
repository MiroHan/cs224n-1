...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, enc_hidden_lens = pad_packed_sequence(enc_hiddens, batch_first=True)
        assert enc_hiddens.shape == (X.shape[1], X.shape[0], self.hidden_size * 2), "kekeke"
        # last_hidden and last_cell both have forward and backward state so we need to concatenate them.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        assert enc_hiddens_proj.shape == (batch_size, enc_hiddens.size(1) ,self.hidden_size), "enc_hiddens_proj wrong shape"
        # 2. Construct tensor `Y` of target sentences with shape (tgt_len, b, e) using the target model embeddings.
        # where tgt_len = maximum target sentence length, b = batch size, e = embedding size.

        Y = self.model_embeddings.target.weight[target_padded]
        assert Y.shape == (target_padded.shape[0], target_padded.shape[1], self.model_embeddings.embed_size), "Y wrong shape"
